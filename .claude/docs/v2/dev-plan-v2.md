...
## Current Sprint
**Sprint 3 — complete 2026-05-16**

v2 feature-complete. All planned tasks shipped. TASK-201 (plan-page next-week polish) was the final task — completed 2026-05-16. Next work is v3 planning.

Scope expanded mid-sprint: full tabloid redesign of all remaining pages added as Phase 2F (dashboard, plan, chat, settings, landing). TASK-133 + Phase 2F UX tasks ran in parallel.
...
**2026-05-16 — Two post-TASK-201 bugs fixed (no task IDs).**

- **Weekly review comparing wrong plan (backend):** `generate_weekly_review()` in `services/review.py` queried the active plan with only `is_active=True + order_by(created_at.desc())`. On weekends, TASK-201 leaves a next-week plan as the most recent active plan, so the review compared this week's actual runs against next week's sessions. Fixed by adding `TrainingPlan.week_start_date == week_start` to the query filter (dropping the now-unnecessary `order_by`). Fallback when no plan exists for the current week was already handled by existing helpers. 15/15 backend tests passing.

- **Dashboard stats comparing wrong plan (frontend):** `dashboard/page.tsx` computed `plannedRunsThisWeek` from whatever `GET /plan/current` returned — same root cause as above. Fixed by computing this week's Monday in UTC (matching the backend's `datetime.now(timezone.utc).date()`) and guarding the stat: if `planData.week_start_date` doesn't match, `plannedRunsThisWeek` is null and the "Runs: X / N" stat shows no target. UTC arithmetic uses `getUTCDay/FullYear/Month/Date` to avoid a local→UTC shift that would produce a wrong date for WIB users between 00:00–07:00. 168/168 frontend tests passing.

**2026-05-16 — TASK-201 complete. v2 feature-complete.** All 11 sub-tasks shipped: week-target resolution helper (`_resolve_target_week_start` with 3-branch rule), week-aware prompt update, `GET /plan/next-target` endpoint, pre-generation preview state, dynamic button/caption copy driven by `reason`, "Next Edition" section label with suppressed today-row for future weeks, replace-confirmation modal (fires only when `replaces_active_plan=true`), Pak Har voice copy for all three reason variants, visual spec for future-week plan and modal. Full SQA coverage: 7 backend rule-branch tests + timezone edge case, component tests for all UI variants, E2E spec covering both the no-active-plan and replace-confirmation flows.
...
### ✅ Done — TASK-201: Plan-page next-week polish (completed 2026-05-16)

> **Goal:** When the user clicks "Generate plan", detect whether they should be planning for *this* week or *next* week, and surface that clearly before they commit. Today the generator always targets the current Monday regardless of how far into the week the runner is or whether they've already trained — producing plans for days that have already passed.

**Resolved rule:**
- Saturday or Sunday → always target next week
- Mon–Fri with any synced run already this week → target next week
- Mon–Fri with no runs yet this week → target this week
- A replace-confirmation modal fires only when an `active` `TrainingPlan` row exists with `week_start_date` equal to the resolved target week
- No override functionality (intentional — keeps the mental model simple)

**Decisions logged 2026-05-16:**
1. Trigger rule: softer hybrid (above) — Friday is still a "salvage" day if no runs yet, weekends always next week
2. No override — just clear messaging via Pak Har voice copy
3. Replace existing active plan with confirmation modal (only when same target week)

#### Backend
- [x] **TASK-201-A1: Week-target resolution helper** → Backend — completed 2026-05-16
  - New `_resolve_target_week_start(user, db) -> tuple[date, str]` returning `(week_start, reason)` where reason ∈ `{"current_week", "weekend", "already_ran_this_week"}`
  - Replaces direct calls to `_get_week_start()` in `generate_plan_with_ollama()`

- [x] **TASK-201-A2: Week-aware prompt update** → Backend — completed 2026-05-16
  - Replace hardcoded `"Generate my training plan for this week."` (services/plan.py:593) with dynamic text built from the resolved target week + reason
  - Acknowledge weekend / already-started cases in the user-side prompt

- [x] **TASK-201-A3: Expose target-week metadata via API** → Backend — completed 2026-05-16
  - SSE `complete` event payload includes `target_week_reason: str` and `is_next_week: bool`
  - New endpoint `GET /plan/next-target` — lightweight read returning `{week_start_date, is_next_week, reason, replaces_active_plan: bool}` for pre-generation preview
  - Update `api-spec-v2.md`

#### Frontend
- [x] **TASK-201-F1: Pre-generation preview state** → Frontend — completed 2026-05-16
  - Plan page calls `GET /plan/next-target` on mount; re-fetches when activities query invalidates
  - Surfaces resolved week + reason above the generate button before the user commits

- [x] **TASK-201-F2: Dynamic button + caption text** → Frontend — completed 2026-05-16
  - Button label: `"File this week's plan"` vs `"File next week's plan"` based on `is_next_week`
  - Caption driven by `reason` (copy from C1)

- [x] **TASK-201-F3: Plan page display label** → Frontend — completed 2026-05-16
  - PlanPaper header: "This Edition · Week of X" vs "Next Edition · Week of X" based on `week_start_date` vs today's Monday
  - No "today" accent row when displaying a future week

- [x] **TASK-201-F4: Replace-confirmation modal** → Frontend — completed 2026-05-16
  - Modal fires only when `next-target.replaces_active_plan === true`
  - Tabloid styling matching existing reset confirmations
  - On confirm: triggers existing generate flow

#### UX
- [x] **TASK-201-C1: Pak Har voice copy** → UX — completed 2026-05-16
  - Three reason-based caption variants (current_week / weekend / already_ran_this_week)
  - Replace-confirmation modal copy
  - 4 new voice test cases added to `pak_har_voice_tests.md`

- [x] **TASK-201-C2: Visual spec for future-week plan + modal** → UX — completed 2026-05-16
  - PlanPaper variant when `is_next_week`: "Next Edition" section label, no accent today-row
  - Confirmation modal: tabloid style (paper bg, 1px ink border, hard corners, Work Sans caps)

#### SQA
- [x] **TASK-201-D1: Backend tests for week resolution** → SQA — completed 2026-05-16
  - All 7 day/activity combinations from the rule table
  - Timezone edge case (midnight UTC)
  - `GET /plan/next-target` response shape including `replaces_active_plan`

- [x] **TASK-201-D2: Frontend component tests** → SQA — completed 2026-05-16
  - Button label switches based on `is_next_week`
  - Caption renders correct copy for each `reason`
  - PlanPaper header renders "Next Edition" when `is_next_week` true
  - Modal opens only when `replaces_active_plan` true

- [x] **TASK-201-D3: E2E test** → SQA — completed 2026-05-16
  - Seed activity for today (Tuesday) → click generate → no modal → next-week plan appears with correct dates
  - Existing active plan for resolved target week → click generate → modal appears → confirm → plan replaced
...
Phase 2 (after A1):
  Backend → A2 + A3
  SQA     → D1 (written against A1 contract)

Phase 3 (after A3 ships + C1/C2 ship):
  Frontend → F1 + F2 + F3 + F4 (single session)

Phase 4 (after F1-F4 ship):
  SQA → D2 + D3
```

**Critical path:** A1 → A3 → F1/F4 → D3 → ship

---

### ✅ Phase 2K — Animation Reel + Strava Connected Screen (completed 2026-05-15)

**TASK-200: Seven physical-medium animations + StravaConnectedStamp success screen**
- `globals.css`: 7 keyframe blocks added (`ol-tw-line-in`, `ol-paper-drop`, `ol-rail-stretch`, `ol-fade-up`, `ol-masthead-settle`, `ol-coach-token-in`, `ol-weekly-fill`, `ol-stamp-land/shake`). Single reduced-motion block at bottom: decorative animations disabled, `ol-weekly-fill` shortened to 200ms, `ol-cursor` stopped.
- `ThinkingPage.tsx`: accepts `steps?: ProgressStep[]` from `useProgressStream`; when provided, drives the step list from real SSE progress instead of the 900ms fake timer. Each step line uses `ol-tw-line` for staggered 150ms fade-up entrance. Fallback (no prop) unchanged.
- `LandingPage.tsx`: `ol-paper-drop` class on root — 600ms entrance on page load.
- `FrontPage.tsx`: weekly km bars animate fill left-to-right (`ol-weekly-fill`, `--ol-fill-target` CSS var, 60ms stagger per row). Current week in accent, others in ink.
- `Dispatch.tsx`: same `ol-weekly-fill` pattern applied to "Last 4 Weeks · KM" rail in the right column.
- `ChatPaper.tsx`: streaming assistant tokens fade up word-by-word (`ol-coach-token`, 18ms stagger per token). Completed messages render as plain text — no replay on re-render.
- `useCountUp.ts` (new): counts numeric value 0→target over 900ms with mechanical ease-out; jumps to target under reduced-motion. Not yet wired to stats strips (deferred).
- `StravaConnectedStamp.tsx` (new): rubber-stamp success view. Stamp lands with 2.4×→1× scale overshoot + rotation; parent shakes one frame after. `onDone` fires after ~1.4s hold.
- `OnboardingModal.tsx`: dark-theme bug fixed — replaced hardcoded hex token object with CSS-var references (`var(--color-paper)` etc.); modal box gets `ol-paper-drop` entrance.
- `auth/callback/route.ts`: redirects to `/auth/connected` instead of `/dashboard` on successful OAuth.
- `auth/connected/page.tsx` (new): server component reads `session_user_id` cookie, passes subscriber number + date to client island.
- `auth/connected/_client.tsx` (new): wires `StravaConnectedStamp.onDone` → `router.replace('/dashboard')`.
