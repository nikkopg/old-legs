"""
Domain context builders for Pak Har coaching prompts.

Pure domain logic that constructs plain-text context strings from DB data
for injection into LLM prompts. Separated from the Ollama HTTP client
(services/ollama.py) so the v3 agent tools layer can import these builders
directly without coupling to the LLM transport layer.

Functions:
    format_pace              — float min/km → "M:SS" display string
    goal_event_label         — goal_event key → human-readable label
    build_strava_context     — last-4-weeks activity summary
    build_user_preferences_context — user goals, schedule, HR data
    build_plan_context       — active weekly training plan summary
    build_voice_modifier     — tonal modifier block for system prompt injection
"""
